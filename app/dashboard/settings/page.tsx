            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={async (e) => {
                  setTwoFaError('')
                  setSuccessMsg('')
                  // optimistic UI: disable input while request runs
                  if (e.target.checked) {
              ...
              />
              <span
                className={`w-14 h-8 rounded-full transition-colors ${profile.twoFactorEnabled ? 'bg-[#69c0d2]' : 'bg-white'}`}
              />
              <span
                className={`absolute left-1 top-1 w-6 h-6 bg-[#050D1A] rounded-full shadow transform transition-transform border border-gray-300 ${profile.twoFactorEnabled ? 'translate-x-6' : ''}`}
              />
            </label>

            {/* <div className="text-sm">
              <div className="font-semibold text-white">{profile.twoFactorEnabled ? '2FA Enabled' : '2FA Disabled'}</div>
              <div className="text-gray-400">{profile.twoFactorEnabled ? 'Two-factor authentication is active for your account.' : 'Add an extra layer of security to your account.'}</div>
            </div> */}
          </div>
        </div>

        {successMsg && (
          <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-xl text-green-400 text-sm font-medium relative">
            <button
              onClick={() => setSuccessMsg('')}
              aria-label="Dismiss"
              className="absolute right-3 top-3 text-green-300 hover:text-green-200"
            >
              ×
            </button>
            {successMsg}
